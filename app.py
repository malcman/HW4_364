...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/malcHW4db" 
...
## Association tables
# NOTE: You may want to complete the models tasks below BEFORE returning to build the association tables! That will making doing this much easier.
...
user_collection = db.Table('user_collection',
	db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')),
	db.Column('personalGifs_id', db.Integer, db.ForeignKey('personalGifs.id'))
	)
...
class User(UserMixin, db.Model):
	...
	#In order to complete a relationship with a table that is detailed below (a one-to-many relationship for users and gif collections), you'll need to add a field to this User model. (Check out the TODOs for models below for more!)
	# Remember, the best way to do so is to add the field, save your code, and then create and run a migration!
	userGifs = db.relationship('PersonalGifCollection', backref='User')
...
# Read through all the models tasks before beginning them so you have an understanding of what the database structure should be like. Consider thinking about it as a whole and drawing it out before you write this code.
...
class PersonalGifCollection(db.Model):
	...
	# Add code for the PersonalGifCollection model such that it has the following fields:
	# id (Integer, primary key)
	id = db.Column(db.Integer, primary_key=True)
	...
	cGIFs = db.relationship('Gif',secondary=user_collection,backref=db.backref('personalGifs',lazy='dynamic'),lazy='dynamic')


class SearchTerm(db.Model):
	__tablename__ = 'searches'
	# Add code for the SearchTerm model such that it has the following fields:
	# id (Integer, primary key)
	id = db.Column(db.Integer, primary_key=True)
	...
	searchGIFs = db.relationship('Gif',secondary=tags,backref=db.backref('searches',lazy='dynamic'),lazy='dynamic')    
...
def get_gifs_from_giphy(search_string):
	...
	# 364: This function should make a request to the Giphy API using the input search_string, and your api_key (imported at the top of this file)
	...
	giphyDict = json.loads(requests.get(baseurl, params = params).text)
	gifObjs = []
	for g in giphyDict['data']:
		gTitle = g['title']
		gEmbedURL = g['embed_url']
		gifObjs.append((gTitle, gEmbedURL))
	return gifObjs
...
def get_or_create_gif(title, url):
	...
	# This function should get or create a Gif instance. Determining whether the gif already exists in the database should be based on the gif's title.
...
def get_or_create_search_term(term):
	...
	giphys = get_gifs_from_giphy(term)
	for g in giphys:
		s.searchGIFs.append(get_or_create_gif(*g))
	db.session.add(s)
	db.session.commit()
	return s
...
def get_or_create_collection(name, current_user, gif_list=[]):
	...
	gifC = PersonalGifCollection.query.filter_by(name = name, user_id = current_user.id).first()
	if not gifC:
		gifC = PersonalGifCollection(name = name, user_id = current_user.id, cGIFs = [])
		for g in gif_list:
			print('g: {}'.format(g))
			print('g type: {}'.format(str(type(g))))
			gifC.cGIFs.append(g)
		db.session.add(gifC)
		db.session.commit()
	return gifC
...
def index():
	# Edit this view function, which has a provided return statement, so that the GifSearchForm can be rendered.
	...
	if form.validate_on_submit():
		...
		get_or_create_search_term(searchTerm)
		return redirect(url_for('search_results', search_term = searchTerm))
...
def search_results(search_term):
	...
	relevant_gifs = term.searchGIFs.all()
	return render_template('searched_gifs.html',gifs=relevant_gifs,term=term)

@app.route('/search_terms')
def search_terms():
	# Edit this view function so it renders search_terms.html.
	...
	return render_template('search_terms.html', all_terms = SearchTerm.query.all())
...
def collections():
	...
	collects = PersonalGifCollection.query.filter_by(user_id = current_user.id).all()
	userCollects = []
	if collects != None:
		userCollects = collects
	return render_template('collections.html', collections = userCollects)
